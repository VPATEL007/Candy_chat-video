...
class _FlutterSliderState extends State<FlutterSlider>
      ...
      if (widget.hatchMark.labels.length > 0) {
        for (FlutterSliderHatchMarkLabel markLabel in widget.hatchMark.labels) {
          ...
          if (tr * hatchMark.density == p) {
            var labelData;
            if (tr >= _outputLowerValue && tr <= _outputUpperValue) {
              labelData = Text(
                markLabel.label.data,
                style: widget.hatchMark.activeStyle,
              );
            } else {
              labelData = Text(
                markLabel.label.data,
                style: widget.hatchMark.inactiveStyle,
              );
            }
            label = labelData; //markLabel.label;
  ...
  void _setParameters() {
    ...
    _handlersWidth = widget.handlerWidth ?? widget.handlerHeight ?? 18;
    _handlersHeight = widget.handlerHeight ?? widget.handlerWidth ?? 18;
  ...
  void _generateHandler() {
    ...
    inputRightHandler.child ??= Container(
      decoration: BoxDecoration(
...
//    }
  ...
  Positioned _inactiveTrack() {
    ...
    Color trackBarColor = boxDecoration.color ?? widget.unselectedColor;
    if (widget.disabled)
    ...
    if (widget.axis == Axis.horizontal) {
      bottom = 0;
      left = _handlersPadding - 15 / 2;
      width = _containerWidthWithoutPadding + 15;
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: List<Widget>.generate(
                      widget.hatchMark.labels.length, (index) {
                    return Container(
                      width: getSize(15),
                      height: getSize(15),
                      decoration: BoxDecoration(
  ...
  Positioned _activeTrack() {
    ...
    Color trackBarColor = boxDecoration.color ?? widget.selectedColor;
    if (widget.disabled)
    ...
    if (widget.axis == Axis.horizontal) {
      ...
      if (!widget.centeredOrigin || widget.rangeSlider) {
        width = _rightHandlerXPosition - _leftHandlerXPosition + 15;
        left =
            _leftHandlerXPosition + _handlersWidth / 2 + (_touchSize) - 15 / 2;
    ...
    int dataMin = _outputLowerValue.toInt();
    int dataMax = _outputUpperValue.toInt();

    int change = (dataMax - dataMin) ~/ widget.step;
    change = change ??= 0;
    return Positioned(
      left: left,
      right: right,
          ...
          children: <Widget>[
            if (!widget.hatchMark.disabled)
              Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: List<Widget>.generate(change + 1, (index) {
                    return Container(
                      width: getSize(15),
                      height: getSize(15),
                      decoration: BoxDecoration(
...
class FlutterSliderTrackBar {
  ...
  const FlutterSliderTrackBar({
    ...
    this.activeTrackBarHeight = 6,
    this.inactiveTrackBarHeight = 6,
    this.centralWidget,
